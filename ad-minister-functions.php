...
function array_rand_weighted( array $values, array $weights ) {
	...
	foreach ( $values as $key => $value ) {
		$weight_sum += $weights[$key] ? $weights[$key] : 1;
...
const GOOGLE_ADSENSE_AD_SLOT_IDS = array (
	...
	'Home Content Premium Banner' => '3888745367',
...
function administer_get_stats( $id = NULL ) {
	...
	if ( isset( $id ) && isset( $administer_stats[$id] ) )
...
function administer_default_editor_to_html ($type) {
	global $page_hook;
	if ( isset( $page_hook ) && strpos( $page_hook, 'ad-minister' ) )
		$type = 'html';
	return $type;
}
...
function administer_get_impressions( $id ) {
	...
	$impressions = isset( $stats['i'] ) ? $stats['i'] : 0;
	return $impressions;
}

function administer_get_clicks( $id ) {
	$stats = administer_get_stats( $id );
	$impressions = isset( $stats['c'] ) ? $stats['c'] : 0;
	return $impressions;
}
